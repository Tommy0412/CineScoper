...
function MoviesContent() {
    ...
    useEffect(() => {
        if (sort === 'popular') {
            fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${key}&include_adult=false&include_null_first_air_dates=false&language=en-US&page=${page}&sort_by=vote_average.desc&vote_count.gte=500&without_genres=16`)
            .then((res) => res.json())
            .then((data) => setData(data.results));
            document.title = 'embedsito - Popular Movies & TV Shows watch online';
            ...
            .then((res) => res.json())
            .then((data) => setData(data.results));
            document.title = 'embedsito - Top Rated Movies & TV Shows watch online';
        }
        window.scrollTo(0,0);
    }, [sort, page]);
